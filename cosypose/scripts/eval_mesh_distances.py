from cosypose.lib3d.distances import dists_add_symmetric
...
from cosypose.config import EXP_DIR, MEMORY, RESULTS_DIR, LOCAL_DATA_DIR, PROJECT_DIR
...
def points_from_mesh(mesh_db, labels):
...
def main():
    ...
    # Used to optimize time and evaluate
    PREDICTIONS_DIR = PROJECT_DIR / "predictions"
    ds_name_underscore = ds_name.replace(".", "_")
    PREDICTIONS_PATH = PREDICTIONS_DIR / f"predictions_{ds_name_underscore}.pt"
    ...
    if not SKIP_PREDICITONS_IF_EXIST or os.path.exists(PREDICTIONS_PATH) is False:
        ...
        torch.save(all_predictions, PREDICTIONS_PATH)
    ...
    el = scene_ds_pred[0]
    el_0 = el[0].squeeze().numpy()
    el_1 = el[1].squeeze().numpy()

    # 'ycbv.bop-compat.eval'
    object_ds = make_object_dataset('ycbv.bop-compat.eval')
    mesh_db = MeshDataBase.from_object_ds(object_ds)
    meshes = mesh_db.batched().cuda().float()
    all_labels = sorted(all_labels, key=lambda lbl : meshes.infos[lbl]["n_points"])
    # conda_pref = os.environ['CONDA_PREFIX']
    print("ds_name: ", ds_name)
    for i, l in enumerate(all_labels):
        # l = all_labels[16]
        n_points = meshes.infos[l]["n_points"]
        print(f"label: {l} n_points:{n_points}")

        single_pointcloud = meshes.select([l]).tensors["points"][:, :n_points]
        TWO2 = TWO1 = torch.zeros(size=(1, 4, 4), device="cuda")

        # print(n_points)
        # print(f"label: {l} i_smallest:{i+1} n_points:{n_points}")
        dists_add_symmetric(TWO1, TWO2, single_pointcloud)
        # Use `bottleneck` to analyze memory usage
        # mem = bn.traced.g
        # print(f"Memory usage of traced function: {mem / 1024 / 1024:.2f} MB")
        # optimize the traced graph for inference and get memory estimate

    return # DBG return

    print("creating EvalRunner (loading data)")
